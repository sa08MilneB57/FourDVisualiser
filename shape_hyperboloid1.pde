...
FourShape hyperboloid(float radius,int detailLon,int detailLat){return hyperboloid(new FourVector(0,0,0,0),radius,detailLon,detailLat,detailLat/2);}
FourShape hyperboloid(float radius,int detailLon, int detailLat,int detailLet){return hyperboloid(new FourVector(0,0,0,0),radius,detailLon,detailLat,detailLet);}
FourShape hyperboloid(FourVector center, float radius,int detailLon, int detailLat,int detailLet){
  ...
  for (int k=0; k<detailLet;k++){
    ...
    for (int j=0; j<detailLat;j++){
      ...
      for (int i=0; i<detailLon;i++){
        ...
        points[index] = lorentzCoordinates(lon,lat,let,radius).add(center);
        if (i==detailLon-1){
