FourShape spherinder(float radius,float h,int detailLon, int detailLat,int detailH){return spherinder(new FourVector(0,0,0,0),radius,h,detailLon,detailLat,detailH);}
FourShape spherinder(FourVector center, float radius,float h,int detailLon, int detailLat,int detailH){
  ...
  for (int k=0; k<detailH;k++){
    ...
    for (int j=0; j<detailLat;j++){
      ...
      for (int i=0; i<detailLon;i++){
        ...
        points[index] = new FourVector(radius*cos(lon)*sin(lat),radius*sin(lon)*sin(lat),radius*cos(lat),w).add(center);
        if (i==detailLon-1){
