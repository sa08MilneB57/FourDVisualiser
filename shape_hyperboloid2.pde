...
FourShape hyperboloid2(float radius,int detailLon,int detailLat){return hyperboloid2(new FourVector(0,0,0,0),radius,detailLon,detailLat,detailLat/2);}
FourShape hyperboloid2(float radius,int detailLon, int detailLat,int detailLet){return hyperboloid2(new FourVector(0,0,0,0),radius,detailLon,detailLat,detailLet);}
FourShape hyperboloid2(FourVector center, float radius,int detailLon, int detailLat,int detailLet){
  ...
  float letstep = (float)(2*Math.PI/(detailLet-1));
  float latstep = (float)(2*Math.PI/(detailLat-1));
  float lonstep = (float)(2*Math.PI/detailLon);
  int detail2 = detailLat*detailLon;
  for (int k=0; k<detailLet;k++){
    float let = k*letstep - PI;
    for (int j=0; j<detailLat;j++){
      float lat = j*latstep - PI;
      for (int i=0; i<detailLon;i++){
        ...
        float w = radius*(float)Math.sinh(let);
        float x = radius*(float)Math.cosh(let)*(float)Math.sinh(lat);
        float y = radius*(float)Math.cosh(let)*(float)Math.cosh(lat)*cos(lon);
        float z = radius*(float)Math.cosh(let)*(float)Math.cosh(lat)*sin(lon);
        points[index] = new FourVector(x,y,z,w).add(center);
        if (i==detailLon-1){
